...
<ol class="breadcrumb">
  <li class="active"><a href="index.php?p=admin.incidents.dashboard">Accueil</a></li>
</ol>

<div class="panel panel-default">
    <div class="panel-heading panel-heading-divider">
        <div class="row">
            <div class="col-md-6">Situation sécuritaire</div>
            <div class="col-md-6"></div>
        </div>
    </div>
    <div class="panel-body" style="margin-top: 10px; margin-left:10px;">
        <div class="row map" id="map">
            <div class="col col-4 col-sm-4 map__list">
                    <ul>
                        <?php foreach($CountryRegions as $CountryRegion): ?>
                            <?php $nbr = 0 ?>
                            <?php foreach($RegionIncidents as $RegionIncident): ?>
                                <?php if ($CountryRegion->region_name == $RegionIncident->region_name && $CountryRegion->id == $RegionIncident->region_id) {
                                    $nbr++;
                                }
                                ?>
                            <?php endforeach; ?>
                            <li><a href="index.php?p=admin.incidents.region&region_name=<?= $CountryRegion->region_name; ?>" id="list-<?= $CountryRegion->region_name; ?>"> <?= $CountryRegion->region_name; ?> (<?= $nbr; ?>)</a></li>
                            <?php if ($nbr <= 2 && $nbr > 0) {
                                    $color = '#ba8b00';

                                    switch ($CountryRegion->region_name) {
                                        case 'Bamako':
                                            $style_bamako = 'fill:'.$color.'';
                                            break;
                                        case 'Kayes':
                                            $style_kayes = 'fill:'.$color.'';
                                             break;
                                        case 'Koulikoro':
                                            $style_koulikoro = 'fill:'.$color.'';
                                             break;
                                        case 'Sikasso':
                                            $style_sikasso = 'fill:'.$color.'';
                                             break;
                                        case 'Segou':
                                            $style_segou = 'fill:'.$color.'';
                                             break;
                                        case 'Mopti':
                                            $style_mopti = 'fill:'.$color.'';
                                             break;
                                        case 'Tombouctou':
                                            $style_tombouctou = 'fill:'.$color.'';
                                             break;
                                        case 'Gao':
                                            $style_gao = 'fill:'.$color.'';
                                             break;
                                        case 'Kidal':
                                            $style_kidal = 'fill:'.$color.'';
                                             break;
                                    }
